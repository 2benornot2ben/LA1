...
/*
 * LAST UPDATED 26 Feb, 11 p.m:
 * Every functionality is working.
 * Future changes: fix errors if there is some. Fix duplicated code. Add unit-tests. Create a jar file.
 */
	...
	public View() throws FileNotFoundException{
		...
		while (running) {
			...
			if (minimize) {
				...
				System.out.println("Enter: \"Rate\" to rate a song."); // This will also handle FAVORITING
			...
			if (holdInputLower.split(" ")[0].equals("search") && holdInputLower.split(" ").length > 1 &&
				...
				String artist = getInput.nextLine();
				ArrayList<Song> songs = myLibrary.searchByIndicatorSong(title, "library", "title");
				if(songs.size() == 0) {
